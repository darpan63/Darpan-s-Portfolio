...
import { Code, Palette, Globe, Server, Database, GitBranch, Briefcase } from "lucide-react";
  ...
  return (
    <section id="about" className="bg-portfolio-dark py-20">
      <div className="section-container">
        ...
        {/* Experience Section */}
        <div className="mt-16">
          <h2 className="text-3xl font-bold text-white mb-8 border-b border-portfolio-primary/30 pb-2">Professional Experience</h2>
          <div className="space-y-8">
            {experience.map((exp, index) => (
              <div key={index} className="bg-white/5 p-6 rounded-lg hover:shadow-lg hover:shadow-portfolio-primary/10 transition-all duration-300">
                <div className="flex items-start mb-4">
                  <div className="bg-portfolio-primary/20 p-3 rounded-full mr-4">
                    <Briefcase className="text-portfolio-primary" size={24} />
                  </div>
                  <div>
                    <h3 className="text-xl font-semibold text-portfolio-primary">{exp.position}</h3>
                    <p className="text-white/90">{exp.company} | {exp.location}</p>
                    <p className="text-white/70">{exp.period}</p>
                    <p className="text-white/80 text-sm mt-1 italic">{exp.technologies}</p>
                  </div>
                </div>
